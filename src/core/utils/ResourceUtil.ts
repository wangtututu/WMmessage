...
class ResourceUtil {
    ...
    public resourceRootPath: string;
     ...
     * @param $onResourceLoadTarget 资源加载监听函数所属对象
     */
    public loadResource($resources = [], $onResourceLoadComplete: Function = null, $onResourceLoadProgress: Function = null, $onResourceLoadTarget: any = null): void {
    ...
    public loadPicAsyn(path: string, bm: egret.Bitmap, width?: number, height?: number): void {
        RES.getResByUrl(path, (data) => {
            bm.texture = data;
            if (width) {
                bm.width = width;
                bm.height = height;
    ...
    public loadPicForImageAsyn(path: string, img: eui.Image, width?: number, height?: number): void {
        RES.getResByUrl(path, (data) => {
            img.source = data;
            if (width) {
                img.width = width;
